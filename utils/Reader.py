...
class AdvancedReader(object):
    ...
    def estimate_class_probs(self):
        self.class_probs = np.zeros([len(self.unique_labels), 1])
    ...
    def next_batch(self, batch_size, normalize=True, shuffle=True, sampling='stratified'):
        ...
        if self.mode == 'train':
            ...
            if sampling == 'stratified':
                ...
                examples_per_class = np.floor(np.divide(batch_size, len(self.unique_labels))) * \
                                     np.ones(shape=[len(self.unique_labels), 1])
        ...
        batch_class_probs = np.zeros([len(self.unique_labels), 1])
        for v in y_batch:
            v = int(v)
    ...
    def __init__(self, source='/gpfs01/berens/user/mayhan/kaggle_dr_data/train_JF_BG_512/',
                 file_type='.jpeg', csv_file='/gpfs01/berens/user/mayhan/kaggle_dr_data/trainLabels.csv', mode='train'
                 ):
        ...
        self.unique_labels = []  # globally unique labels
        ...
        self.unique_labels = np.unique(list(self.data_dict.values()))  # globally unique labels
        if self.mode == 'train':
